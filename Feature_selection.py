...
clf = RandomForestClassifier(n_estimators=666,n_jobs=-1)
...
rfecv = RFECV(clf,cv=StratifiedKFold(10,shuffle=True,random_state=123),scoring='neg_log_loss',step=50,verbose=10)
rfecv.fit(X,y)
pd.DataFrame([X.columns,rfecv.support_,rfecv.ranking_]).to_csv('feature_selection_on_manual.csv')
pd.DataFrame([range(1, len(rfecv.grid_scores_) + 1,3),rfecv.grid_scores_]).to_csv('feature_scores_on_manual.csv')
